...
namespace Ovh\License\Directadmin\Exception;
		...
		switch ($statusCode) {
			case 404 :
				...
				if ($response->getReasonPhrase() == "The requested object (Tasks) does not exist") {
					...
					$taskId = $d[5];
					throw new TaskDoesNotExistsException('There is no task with ID : ' . $taskId . '. for Directadmin ' . $this->getDomain($request->getPath()), 404);
				} else throw $prev;
