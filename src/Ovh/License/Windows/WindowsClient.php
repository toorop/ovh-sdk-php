...
use Ovh\License\Windows\Exception\WindowsException;

class WindowsClient extends AbstractClient
{
   ...
   /**
    ...
    public function getOrderableVersions($domain,$ip)
    {
        ...
        try {
            $r = $this->get('license/windows/' . $domain .'?'.$ip)->send();
        } catch (\Exception $e) {
    ...
    public function getProperties($domain)
    {
		if (!$domain)
			throw new BadMethodCallException('Parameter $domain is missing');
        try {
            $r = $this->get('license/windows/' . $domain)->send();
        } catch (\Exception $e) {
            throw new WindowsException($e->getMessage(), $e->getCode(), $e);
        }
        return $r->getBody(true);
