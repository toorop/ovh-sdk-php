...
namespace Ovh\License\Worklight\Exception;
...
class WorklightException extends \RuntimeException
{
		...
		switch ($statusCode) {
			case 404 :
				...
				if ($response->getReasonPhrase() == "The requested object (Tasks) does not exist") {
					...
					$taskId = $d[5];
					throw new TaskDoesNotExistsException('There is no task with ID : ' . $taskId . '. for Worklight ' . $this->getDomain($request->getPath()), 404);
				} else throw $prev;
