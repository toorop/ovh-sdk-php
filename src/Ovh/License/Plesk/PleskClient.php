...
use Ovh\License\Plesk\Exception\PleskException;

class PleskClient extends AbstractClient
{
    ...
    public function getOrderableVersions($domain,$ip)
    {
        ...
        try {
            $r = $this->get('license/plesk/' . $domain .'?'.$ip)->send();
        } catch (\Exception $e) {
            throw new PleskException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    public function getProperties($domain)
    {
		if (!$domain)
			throw new BadMethodCallException('Parameter $domain is missing');
        try {
            $r = $this->get('license/plesk/' . $domain)->send();
        } catch (\Exception $e) {
            throw new PleskException($e->getMessage(), $e->getCode(), $e);
        }
        return $r->getBody(true);
