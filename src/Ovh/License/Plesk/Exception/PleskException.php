...
namespace Ovh\License\Plesk\Exception;
...
class PleskException extends \RuntimeException
{
		...
		switch ($statusCode) {
			case 404 :
				...
				if ($response->getReasonPhrase() == "The requested object (Tasks) does not exist") {
					...
					$taskId = $d[5];
					throw new TaskDoesNotExistsException('There is no task with ID : ' . $taskId . '. for Plesk ' . $this->getDomain($request->getPath()), 404);
				} else throw $prev;
